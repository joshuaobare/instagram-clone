...
export default function FullPost(props) {
  ...
  useEffect(() => {
    // the content in the component is retrieved by searching for the currentPost id in the profiles array
      ...
      if (items.posts.arrayValue.values) {
        return items.posts.arrayValue.values.find(
          (item) =>
            ...
            props.currentPost.id.stringValue
        );
      } else {
        return null;
      }
    ...
    // an empty likes array throws an error as .arrayValue.values is non-existent
    if (props.currentPost.likes.arrayValue.values) {
      setLikesCount(props.currentPost.likes.arrayValue.values.length);
      const checker = props.currentPost.likes.arrayValue.values.some(
    ...
    // the comments in the currentPost are unpacked and set to the comments state
    const comms = [];
      ...
      props.currentPost.comments.arrayValue.values.forEach((obj) => {
        const profile = props.profiles.find((items) => {
          if (items.posts.arrayValue.values) {
            return items.posts.arrayValue.values.find(
              (item) =>
                item.mapValue.fields.username.stringValue ===
                obj.mapValue.fields.username.stringValue
            );
          } else {
            return null;
          }
    ...
    setDate(
      `${time.getDate()} ${new Intl.DateTimeFormat("en-US", options).format(
        time
      )} ${time.getFullYear()}`
    );
    ...
    >
      <div className="Full-Post">
        <div className="full-post-main-img-cont">
          <img
            src={data.url.stringValue}
            alt="main post"
            className="full-post-main-img"
          />
        </div>
        ...
        <div className="full-post-right-section">
          <div className="full-post-header">
            ...
            <button
              onClick={props.togglePostDialog}
              className="full-post-close-icon"
            >
              <Close />
            </button>
          </div>
            ...
            <div className="full-post-caption-section">
              <div className="full-post-caption-section-main">
                <img
                  ...
                  className="full-post-profile-icon"
                />
                <div className="full-post-caption-username">
                  {data.username.stringValue}
                </div>
              </div>
              <div className="full-post-caption">
                {data.caption.stringValue}
              </div>
            </div>
          ...
          <div className="full-post-addcomment">
            ...
            <form action="" className="full-post-comment-form">
              <input
                ...
                onChange={(e) =>
                  props.handleCommentChange(e, data.id.stringValue)
                }
              />
              <button
